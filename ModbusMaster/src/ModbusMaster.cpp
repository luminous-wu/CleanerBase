...
void ModbusMaster::setSubIndex(uint8_t u8SubIndex) {
  _u8SubIndex = u8SubIndex;
}
...
@ingroup discrete
*/
// uint8_t ModbusMaster::readCoils(uint16_t u16ReadAddress, uint16_t u16BitQty, uint8_t* data)
// {
//   _u16ReadAddress = u16ReadAddress;
//   _u16ReadQty = u16BitQty;
//   return ModbusMasterTransaction(ku8MBReadCoils, uint8_t* data);
// }
...
uint8_t ModbusMaster::readHoldingRegisters(uint16_t u16ReadAddress,
  uint16_t u16ReadQty, uint8_t* data)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModbusMasterTransaction(ku8MBReadHoldingRegisters, data);
}
...
uint8_t ModbusMaster::readInputRegisters(uint16_t u16ReadAddress,
  uint8_t u16ReadQty, uint8_t* data)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModbusMasterTransaction(ku8MBReadInputRegisters, data);
}
...
uint8_t ModbusMaster::writeSingleRegister(uint16_t u16WriteAddress,
  uint16_t u16WriteValue, uint8_t* data)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = 0;
  _u16TransmitBuffer[0] = u16WriteValue;
  return ModbusMasterTransaction(ku8MBWriteSingleRegister, data);
}
...
uint8_t ModbusMaster::writeMultipleRegisters(uint16_t u16WriteAddress,
  uint16_t u16WriteQty, uint8_t* data)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = u16WriteQty;
  return ModbusMasterTransaction(ku8MBWriteMultipleRegisters, data);
}
...
@return 0 on success; exception number on failure
*/
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction,uint8_t* data)
{
...
uint8_t ModbusMaster::setVelocity(uint8_t* data, uint8_t* u8ModbusADU) {

  uint8_t u8MBFunction = 0x10;
  // u8ModbusADU[256] = { 0 };
  uint8_t i, u8Qty;
  uint16_t u16CRC;
  uint32_t u32StartTime;
